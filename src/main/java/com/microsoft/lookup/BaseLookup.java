...
public abstract class BaseLookup<T extends Element> {
    ...
    protected String determineType(T element) {
        ...
        return ElementKindLookup.getNameByKind(element.getKind());
    }
    ...
    public enum ElementKindLookup {
        PACKAGE(ElementKind.PACKAGE, "Namespace"),
        CLASS(ElementKind.CLASS, "Class"),
        ENUM(ElementKind.ENUM, "Enum"),
        ENUM_CONSTANT(ElementKind.ENUM_CONSTANT, "Field"),
        INTERFACE(ElementKind.INTERFACE, "Interface"),
        ANNOTATION_TYPE(ElementKind.ANNOTATION_TYPE, "Interface"),
        CONSTRUCTOR(ElementKind.CONSTRUCTOR, "Constructor"),
        METHOD(ElementKind.METHOD, "Method"),
        FIELD(ElementKind.FIELD, "Field");

        private final ElementKind kind;
        private final String      name;

        ElementKindLookup(ElementKind kind, String name) {
            this.kind = kind;
            this.name = name;
        }

        public static String getNameByKind(ElementKind kind) {
            String val = Stream.of(ElementKindLookup.values())
                    .filter(lookup -> lookup.getKind().equals(kind))
                    .map(ElementKindLookup::getName)
                    .findFirst()
                    .orElse(null);
            if (val == null) {
                throw new NullPointerException(String.format("No such element kind: %s", kind));
            }

            return val;
        }

        public static String determineType(TypeElement classElement) {
            return getNameByKind(classElement.getKind());
        }
